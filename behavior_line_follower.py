	...
	def sense_and_act(self, reflactance_values):
		...
		if self.active_flag:
			...
			if l > self.THRESHOLD or r > self.THRESHOLD:
				...
				if l < self.THRESHOLD:
					self.motor_recommandations = [("l",0.3,0.1)]
				else:
					self.motor_recommandations = [("r",0.3,0.1)]
				return self.motor_recommandations
			...
			if l > self.THRESHOLD or r > self.THRESHOLD:
				...
				if l < self.THRESHOLD:
					self.motor_recommandations = [("l",0.6,0.5)]
				else:
					self.motor_recommandations = [("r",0.6,0.5)]
				return self.motor_recommandations
			...
			if l > self.THRESHOLD or r > self.THRESHOLD:
				...
				if l < self.THRESHOLD:
					self.motor_recommandations = [("l",0.9,1)]
				else:
					self.motor_recommandations = [("r",0.9,1)]
				return self.motor_recommandations
			#Not on line
			else: 
